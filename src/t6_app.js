...
/* ============ STATE ============ */
let state={start:"2026-09-11",daily:40,route:[],sel:"lima",filterCCs:[],filterTags:[],ddOpen:null,labels:false,insertAt:null,slots:{A:null,B:null},wx:false,wxMonth:10,anchors:[],done:[],lock:false,pcol:false,pw:460,cmpA:null,cmpB:null,picks:{},saved:[],cmpSaved:[],legMode:{},checks:{},packDone:{},acts:{},night:{}};
...
function schedule(route){
  ...
  let cur=startDate(),cost=0,nights=0,legDays=0,hostel=0,flights=0,busH=0,km=0,acts=0,actDays=0,nightN=0,nightSaved=0;const actSeen=new Set();
  const rows=[];
  seq.forEach((r,i)=>{
    let lg=null;
    ...
    hostel+=r.days*BY[r.id].dorm;nights+=r.days;if(lg&&lg.m==="bus"&&state.night[seq[i-1].id+">"+r.id]){hostel-=BY[r.id].dorm;nightN++;nightSaved+=BY[r.id].dorm;r.night=true;}
    if(!actSeen.has(r.id)){actSeen.add(r.id);ACTS.forEach(x=>{if(x.stop===r.id&&state.acts[r.id+"|"+x.name]){acts+=x.usd;actDays+=x.days||0;}});}
    rows.push({...r,from,to,leg:lg,i});
    cur=addDays(to,1);
  });
  const rio=rows[rows.length-1];
  const total=cost+hostel+acts+availDays()*state.daily;
  ...
  return {rows,transport:cost,nights,legDays,hostel,acts,actDays,nightN,nightSaved,rioDays:rio.days,over:rio.days<1?1-rio.days:0,flights,busH,km,total,ccs,anchors,missed:anchors.filter(a=>!a.ok)};
}
...
const gDefs=el("defs"),gGrat=el("g"),gGlow=el("g"),gCountries=el("g"),gLabels=el("g"),gRoute=el("g"),gPins=el("g"),gWalker=el("g"),gPoi=el("g",{class:"pois"});
svg.append(gDefs,gGrat,gGlow,gCountries,gLabels,gRoute,gPoi,gPins,gWalker);
const POI_ICON={viewpoint:"◉",hike:"▲",beach:"◠",market:"▣",ruins:"⌂",nature:"✿",nightlife:"♪",museum:"▤",food:"●",activity:"✦"};
const poiEls=[];
(typeof POI!=="undefined"?POI:[]).forEach((p,i)=>{if(!BY[p.stop])return;const g=el("g",{class:"poi k-"+(p.kind||"activity")});g.dataset.i=i;
  const c=el("circle",{r:4.2});const t=el("text",{class:"pk",y:0});t.textContent=POI_ICON[p.kind]||"✦";const l=el("text",{class:"pl",y:12});l.textContent=p.name;g.append(c,t,l);
  g.addEventListener("click",e=>{e.stopPropagation();showPoi(i);});g.addEventListener("mouseenter",e=>{if(isMobile())return;tip.className="tip leg";tip.innerHTML=`<div class="tt"><span class="mode">${p.kind||"place"}${p.time?" · "+p.time:""}</span><b>${esc(p.name)}</b><small>${esc(p.note||"")}</small></div>`;moveTip(e);tip.classList.add("show");});g.addEventListener("mousemove",moveTip);g.addEventListener("mouseleave",hideTip);
  gPoi.append(g);poiEls.push(g);});
function showPoi(i){const p=POI[i];select(p.stop,true);const r=svg.getBoundingClientRect();const fr=freeRect();const w=Math.min(vb.w,W*0.06);const sc=w/r.width;animateTo({x:px(p.lon)-(fr.left+fr.width/2)*sc,y:py(p.lat)-(fr.top+fr.height/2)*sc,w,h:w*r.height/r.width},true,700);poiEls.forEach(g=>g.classList.remove("hot"));const g=poiEls.find(g=>+g.dataset.i===i);if(g)g.classList.add("hot");setTimeout(()=>{const el2=document.querySelector(`#tab-stop [data-poi="${i}"]`);if(el2)el2.scrollIntoView({behavior:"smooth",block:"center"});},350);}
...
function setVB(){const rr=svg.getBoundingClientRect();if(!rr.width||!rr.height)return;vb.h=vb.w*rr.height/rr.width;if(![vb.x,vb.y,vb.w,vb.h].every(Number.isFinite)){vb={x:0,y:0,w:W,h:W*rr.height/rr.width};}svg.setAttribute("viewBox",`${vb.x} ${vb.y} ${vb.w} ${vb.h}`);K=vb.w/W;
  ...
  if(Math.abs(K-lastK)>0.004||scale!==lastScale){lastK=K;lastScale=scale;const k=K*scale*(K<1?Math.pow(K,-0.22):1);const kp=K*scale*(K<1?Math.pow(K,-0.1):1);const showP=K<0.09;svg.classList.toggle("z-poi",showP);if(showP)poiEls.forEach(g=>{const p=POI[+g.dataset.i];g.setAttribute("transform",`translate(${px(p.lon)},${py(p.lat)}) scale(${kp})`);});
...
function animateTo(target,anim,ms){if(![target.x,target.y,target.w,target.h].every(Number.isFinite))return;if(!anim||reduced()||document.hidden){vb=target;setVB();return;}const from={...vb};let t0=null;const dur=ms||450;const step=ts=>{if(!t0)t0=ts;const t=Math.min(1,(ts-t0)/dur);const e=1-Math.pow(1-t,3);vb={x:from.x+(target.x-from.x)*e,y:from.y+(target.y-from.y)*e,w:from.w+(target.w-from.w)*e,h:from.h+(target.h-from.h)*e};setVB();if(t<1)requestAnimationFrame(step);};requestAnimationFrame(step);}
...
let story=null;
function openStory(){const S=schedule();if(!state.route.length){toast("Add stops first");return;}const box=document.getElementById("story");box.hidden=false;document.body.classList.add("storying");story={i:0,S,auto:false};
  const render=()=>{const {i,S}=story;const r=S.rows[i];const s=BY[r.id];const lg=r.leg;const w=wxAt(s,r.from.getMonth()+1);const pick=state.picks[s.id];const acts=ACTS.filter(x=>x.stop===s.id&&state.acts[s.id+"|"+x.name]);const gal=(IMG2[s.id]||[]);const imgs=[IMG[s.id],...gal].filter(Boolean);const img=imgs[i%imgs.length]||IMG[s.id];
    box.innerHTML=`<img class="st-bg" src="${img}" alt=""><div class="st-sv"><div class="st-top"><span class="st-mono">${i+1} / ${S.rows.length} · ${COUNTRY[s.cc]||s.cc}</span><div class="st-dots">${S.rows.map((x,j)=>`<i class="${j===i?"on":j<i?"done":""}" data-j="${j}"></i>`).join("")}</div><button class="st-x" id="stX" aria-label="Close">×</button></div>
    <div class="st-mid"><button class="st-nav" id="stPrev" ${i===0?"disabled":""}>‹</button><button class="st-nav" id="stNext" ${i===S.rows.length-1?"disabled":""}>›</button></div>
    <div class="st-bot"><span class="st-eyebrow">${lg?`${MODE_NAME[lg.m]||lg.m} ${lg.h>=24?Math.round(lg.h/24*10)/10+" d":lg.h+" h"} from ${esc(short(BY[S.rows[i-1].id].name))}${r.night?" · night bus":""}`:"Landing"}</span><h1>${esc(s.name)}</h1><p class="st-tag">${esc(s.tag)}</p>
    <div class="st-facts"><span>${fmt(r.from)}${r.days>1?" → "+fmt(r.to):""}</span><span>${r.days} d</span>${w?`<span class="st-wx ${w.r}">${w.hi}° · ${w.label}</span>`:""}${pick?`<span>🛏 ${esc(pick)}</span>`:""}${acts.length?`<span>✦ ${acts.map(x=>esc(x.name)).join(", ")}</span>`:""}</div>
    <div class="st-ctl"><button id="stAuto" class="${story.auto?"on":""}">${story.auto?"⏸ Auto":"▶ Auto"}</button><button id="stMap">Show on map</button></div></div></div>`;
    box.querySelector("#stX").onclick=closeStory;box.querySelector("#stPrev").onclick=()=>go(-1);box.querySelector("#stNext").onclick=()=>go(1);box.querySelector("#stAuto").onclick=()=>{story.auto=!story.auto;tick();render();};box.querySelector("#stMap").onclick=()=>{closeStory();select(s.id,true);focusStop(s.id,W*0.22);};box.querySelectorAll(".st-dots i").forEach(d=>d.onclick=()=>{story.i=+d.dataset.j;render();});
    state.sel=s.id;drawRoute();focusStop(s.id,W*0.22);};
  const go=d=>{story.i=Math.max(0,Math.min(story.S.rows.length-1,story.i+d));render();};const tick=()=>{clearTimeout(story.t);if(story.auto)story.t=setTimeout(()=>{if(!story)return;if(story.i<story.S.rows.length-1){go(1);tick();}else{story.auto=false;render();}},3200);};
  story.key=e=>{if(e.key==="Escape")closeStory();else if(e.key==="ArrowRight"||e.key===" ")go(1);else if(e.key==="ArrowLeft")go(-1);};document.addEventListener("keydown",story.key);
  let tx=null;box.ontouchstart=e=>{tx=e.touches[0].clientX;};box.ontouchend=e=>{if(tx==null)return;const dx=e.changedTouches[0].clientX-tx;if(Math.abs(dx)>50)go(dx<0?1:-1);tx=null;};
  render();}
function closeStory(){if(!story)return;clearTimeout(story.t);document.removeEventListener("keydown",story.key);story=null;const box=document.getElementById("story");box.hidden=true;box.innerHTML="";document.body.classList.remove("storying");renderStop();}
...
function renderStop(){
  ...
  pane.innerHTML=`
   ...
   <div class="detail">
    ...
    ${(()=>{const ps=(typeof POI!=="undefined"?POI:[]).map((p,i)=>({p,i})).filter(x=>x.p.stop===s.id);return ps.length?`<div class="pois-list">${ps.map(({p,i})=>`<button class="poirow" data-poi="${i}"><span class="pkind">${POI_ICON[p.kind]||"✦"}</span><span><b>${esc(p.name)}</b><small>${esc(p.note||"")}${p.time?" · "+esc(p.time):""}</small></span><span class="go">map ↗</span></button>`).join("")}</div>`:`<ul class="hi">${(s.hi||[]).map(h=>`<li>${esc(h)}</li>`).join("")}</ul>`;})()}
    ${actsHTML(s)}
  ...
  pane.querySelectorAll("[data-act]").forEach(b=>b.onclick=()=>{const k=s.id+"|"+b.dataset.act;if(state.acts[k])delete state.acts[k];else state.acts[k]=true;save();dirty.route=true;const on=!!state.acts[k];b.classList.toggle("on",on);b.querySelector(".ck").textContent=on?"✓":"+";const a=ACTS.find(x=>x.stop===s.id&&x.name===b.dataset.act);toast(on?`${a.name} · $${a.usd} added to the budget`:`${a.name} removed`);renderMeter();const hd=pane.querySelector(".acts").previousElementSibling;if(hd){const picked=ACTS.filter(x=>x.stop===s.id&&state.acts[s.id+"|"+x.name]);let c=hd.querySelector(".cnt.on");if(!picked.length){c&&c.remove();}else{if(!c){c=document.createElement("span");c.className="cnt on";hd.append(c);}c.textContent=`${picked.length} in budget · $${picked.reduce((t,x)=>t+x.usd,0)}`;}}});
  pane.querySelectorAll("[data-hood]").forEach(b=>b.onclick=()=>{const norm=x=>(x||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"");const key=norm(b.dataset.hood).split(/[ /(]/)[0];const hit=[...pane.querySelectorAll(".hostel")].filter(el=>norm(el.dataset.area).includes(key));pane.querySelectorAll(".hostel").forEach(el=>el.classList.toggle("dimmed",hit.length>0&&!hit.includes(el)));pane.querySelectorAll(".hood").forEach(el=>el.classList.toggle("sel",el===b&&!b.classList.contains("sel")));if(!b.classList.contains("sel"))pane.querySelectorAll(".hostel").forEach(el=>el.classList.remove("dimmed"));else if(hit[0])hit[0].scrollIntoView({behavior:"smooth",block:"center"});});
  ...
  pane.querySelectorAll("[data-poi]").forEach(b=>b.onclick=()=>showPoi(+b.dataset.poi));
...
/* ============ ROUTE ============ */
function settingsHTML(){return `<div class="settings2"><label class="f"><span>Land in Lima</span><input type="date" id="startDate" value="${state.start}"></label><span class="arr">→</span><span class="f lock"><span>New Year's Eve</span><b>Rio · 31 Dec</b></span><label class="f"><span>Budget / day</span><span class="in">$<input type="number" id="daily" min="10" max="300" step="5" value="${state.daily}"></span></label><span class="days">${availDays()} days</span><label class="f tempo" title="Scale every stay proportionally; Rio stays fixed"><span>Tempo</span><input type="range" id="tempo" min="60" max="140" value="100" step="5"></label><span class="help" title="Daily budget covers food, activities and local transport; beds and long-distance legs are added separately for the total.">?</span><span class="days mono" id="tempoOut"></span></div>`;}
function bindSettings(pane){const tp=pane.querySelector("#tempo");if(tp){let base=null;tp.oninput=()=>{if(!base){base=state.route.map(r=>r.days);snap();}const f=+tp.value/100;state.route.forEach((r,i)=>{r.days=Math.max(1,Math.min(30,Math.round(base[i]*f)));});save();renderMeter();const S=schedule();const o=pane.querySelector("#tempoOut");if(o)o.textContent=`${Math.round(f*100)}% · ${S.rioDays} d in Rio`;const c=pane.querySelector(".summary .fact b");if(c)c.textContent=S.rioDays<1?"−"+S.over:S.rioDays;};tp.onchange=()=>{base=null;commit();toast("Tempo applied",undoLast);};}
const sd=pane.querySelector("#startDate");if(sd)sd.onchange=()=>{if(sd.value){state.start=sd.value;commit();}};const dl=pane.querySelector("#daily");if(dl)dl.onchange=()=>{state.daily=Math.max(10,Math.min(300,+dl.value||40));commit();};}
...
function renderRoute(){
  ...
  if(!state.route.length){pane.innerHTML=`<div class="route-pane">${sharedBanner()}${settingsHTML()}<div class="empty photo"><img src="${IMGM.machu||IMG.machu}" alt=""><div class="et"><h2>Start in Lima, end on Copacabana</h2><p>Tap a stop on the map or in Explore and add it, or load a preset below and bend it to your taste.</p></div></div><div class="presets" id="presetsIn"></div></div>`;bindSettings(pane);renderPresetsInto(pane.querySelector("#presetsIn"));return;}
  ...
  let html=`<div class="route-pane">${sharedBanner()}${settingsHTML()}${todayHTML(S)}
   ...
   <div class="summary">
    ...
    <div class="fact ${S.acts?"":"dim"}" title="Activities you ticked on the stop pages"><small>Activities</small><b>~$${S.acts.toLocaleString()}</b></div>
  ...
  S.rows.forEach((r,i)=>{
    ...
    html+=`<div class="leg ${lg.est?"warn":""}"><div class="ln"><i></i></div><div class="txt"><span class="mode">${MODE_ICON[lg.m]||""}${MODE_NAME[lg.m]||lg.m}${r.night?" ☾ night":""}</span><span class="mono">${lg.h>=24?Math.round(lg.h/24*10)/10+" d":lg.h+" h"}</span><span class="mono">${lg.u?"~$"+lg.u:"incl."}</span>${lg.d?`<span class="mono">+${lg.d} travel day${lg.d>1?"s":""}</span>`:""}${lg.src?`<span class="src">researched</span>`:lg.est?`<span class="src" style="color:var(--warn);border-color:var(--warn)">estimate</span>`:""}${lg.n?`<span class="nt">${esc(lg.n)}</span>`:""}${sw}</div></div>`;}
...
async function exportPNG(){
  const S=schedule();if(!state.route.length){toast("Add stops first");return;}
  const Wp=1600,Hp=1000,pad=60;const ids=[...new Set([...state.route.map(r=>r.id),"rio"])];
  const xs=ids.map(i=>px(BY[i].lon)),ys=ids.map(i=>py(BY[i].lat));let x0=Math.min(...xs)-40,x1=Math.max(...xs)+40,y0=Math.min(...ys)-60,y1=Math.max(...ys)+40;
  const mapH=Hp-260;let sc=Math.max((x1-x0)/(Wp-2*pad),(y1-y0)/(mapH-2*pad));const cx=(x0+x1)/2,cy=(y0+y1)/2;const ox=Wp/2-cx/sc,oy=pad+mapH/2-cy/sc;
  const dark=matchMedia("(prefers-color-scheme: dark)").matches&&document.documentElement.dataset.theme!=="light"||document.documentElement.dataset.theme==="dark";
  const bg=dark?"#0F1D1C":"#EEF0EA",sea=dark?"#0B1A1E":"#D3E2E2",land=dark?"#233533":"#DCE2D6",ink=dark?"#EDE7D6":"#15211F",muted=dark?"#8C9C97":"#6E7E78",acc=dark?"#FF9A3C":"#DB6A1E";
  const c=document.createElement("canvas");c.width=Wp;c.height=Hp;const g=c.getContext("2d");
  g.fillStyle=bg;g.fillRect(0,0,Wp,Hp);g.fillStyle=sea;g.fillRect(pad,pad,Wp-2*pad,mapH);
  g.save();g.beginPath();g.rect(pad,pad,Wp-2*pad,mapH);g.clip();g.translate(ox,oy);g.scale(1/sc,1/sc);
  for(const d of Object.values(GEO.paths)){const p=new Path2D(d);g.fillStyle=land;g.fill(p);g.strokeStyle=dark?"#4A5F5A":"#B7C4BC";g.lineWidth=1.2*sc;g.stroke(p);}
  const seq=[...state.route.map(r=>r.id),"rio"];g.lineWidth=4*sc;g.strokeStyle=acc;g.lineCap="round";
  for(let i=1;i<seq.length;i++){const a=BY[seq[i-1]],b=BY[seq[i]];if(a===b)continue;const lg=leg(a.id,b.id);g.setLineDash(lg.m==="flight"?[10*sc,8*sc]:[]);g.beginPath();g.moveTo(px(a.lon),py(a.lat));if(lg.m==="flight"){const mx=(px(a.lon)+px(b.lon))/2,my=(py(a.lat)+py(b.lat))/2,dx=px(b.lon)-px(a.lon),dy=py(b.lat)-py(a.lat);g.quadraticCurveTo(mx-dy*0.18,my+dx*0.18,px(b.lon),py(b.lat));}else g.lineTo(px(b.lon),py(b.lat));g.stroke();}
  g.setLineDash([]);const order={};seq.forEach((id,i)=>{if(order[id]===undefined)order[id]=i+1;});
  const drawPin=(s,n)=>new Promise(res=>{const im=new Image();im.onload=()=>{const r=14*sc,x=px(s.lon),y=py(s.lat);g.save();g.beginPath();g.arc(x,y,r,0,7);g.clip();g.drawImage(im,x-r,y-r,2*r,2*r);g.restore();g.beginPath();g.arc(x,y,r,0,7);g.lineWidth=3*sc;g.strokeStyle=acc;g.stroke();g.fillStyle=acc;g.beginPath();g.arc(x+r*.8,y-r*.8,7*sc,0,7);g.fill();g.fillStyle="#1A0E05";g.font=`bold ${9*sc}px sans-serif`;g.textAlign="center";g.textBaseline="middle";g.fillText(n,x+r*.8,y-r*.8);g.fillStyle=ink;g.font=`bold ${11*sc}px sans-serif`;g.textBaseline="top";g.strokeStyle=sea;g.lineWidth=4*sc;g.strokeText(short(s.name),x,y+r+4*sc);g.fillText(short(s.name),x,y+r+4*sc);res();};im.onerror=res;im.src=IMGT[s.id]||IMG[s.id];});
  for(const id of ids){await drawPin(BY[id],id==="rio"?"★":order[id]);}
  g.restore();
  g.fillStyle=ink;g.font="bold 44px sans-serif";g.textAlign="left";g.textBaseline="alphabetic";g.fillText("Andes to Copacabana",pad,pad+mapH+64);
  g.fillStyle=muted;g.font="22px sans-serif";g.fillText(`${fmt(startDate())} → ${fmt(NYE)} · ${S.rows.length} stops · ${S.ccs.length} countries · ~$${S.total.toLocaleString()}`,pad,pad+mapH+100);
  g.font="18px sans-serif";g.fillStyle=ink;const names=S.rows.map((r,i)=>`${i+1} ${short(BY[r.id].name)}`);let line="",lines=[];names.forEach(n=>{const t=line?line+"  ·  "+n:n;if(g.measureText(t).width>Wp-2*pad){lines.push(line);line=n;}else line=t;});lines.push(line);lines.slice(0,3).forEach((l,i)=>g.fillText(l,pad,pad+mapH+140+i*28));
  const blob=await new Promise(r=>c.toBlob(r,"image/png"));
  try{const dl=window.claude&&window.claude.use?await window.claude.use("downloads"):null;if(dl){await dl.save({filename:"my-route.png",data:blob});toast("Saved my-route.png");return;}}catch(e){if(e&&e.code==="declined")return;}
  const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="my-route.png";document.body.append(a);a.click();a.remove();toast("Route image downloaded");
}
...
const ACT_ICON={trek:"▲",tour:"◉",water:"≈",adventure:"✦",wildlife:"✿",culture:"▤",nightlife:"♪",class:"●"};
function nightCands(S){return S.rows.filter(r=>r.leg&&r.leg.m==="bus"&&r.leg.h>=7&&r.leg.h<=16&&!r.locked).map(r=>{const prev=S.rows[r.i-1];return {key:prev.id+">"+r.id,from:prev,to:r,h:r.leg.h,save:BY[r.id].dorm,on:!!state.night[prev.id+">"+r.id],hint:/night|overnight|sleeper|cama/i.test(r.leg.n||"")};});}
function nightHTML(S){const c=nightCands(S);if(!c.length)return "";const on=c.filter(x=>x.on);const pot=c.reduce((t,x)=>t+x.save,0);
  return `<details class="night" id="nightD"><summary><span class="ico">☾</span><b>Night buses</b><span class="sub">${on.length?`${on.length} marked · saves ~$${on.reduce((t,x)=>t+x.save,0)} in beds and ${on.length} day${on.length>1?"s":""} of daylight`:`${c.length} leg${c.length>1?"s":""} of 7–16 h could run overnight · up to ~$${pot} and ${c.length} days saved`}</span></summary>
  <div class="nb">${c.map(x=>`<label class="nbrow ${x.on?"on":""}"><input type="checkbox" data-night="${x.key}" ${x.on?"checked":""}><span><b>${esc(short(BY[x.from.id].name))} → ${esc(short(BY[x.to.id].name))}</b><small>${x.h} h${x.hint?" · night service exists":""} · skip one bed in ${esc(short(BY[x.to.id].name))} (~$${x.save})</small></span><span class="mono">${x.on?"☾ night":"day"}</span></label>`).join("")}
  <button class="nbAll" data-nightall="${on.length===c.length?"0":"1"}">${on.length===c.length?"Unmark all":"Mark all as night buses"}</button><p class="small-note">Night buses save a hostel night and keep the days free; the bed cost is removed from the estimate, the arrival day stays as planned. Bring a fleece and a lock.</p></div></details>`;}
function bindNight(pane){pane.querySelectorAll("[data-night]").forEach(cb=>cb.onchange=()=>{snap();state.night[cb.dataset.night]=cb.checked;if(!cb.checked)delete state.night[cb.dataset.night];commit();});const all=pane.querySelector("[data-nightall]");if(all)all.onclick=()=>{snap();const S=schedule();nightCands(S).forEach(x=>{if(all.dataset.nightall==="1")state.night[x.key]=true;else delete state.night[x.key];});commit();};}
function actsHTML(s){const list=ACTS.filter(x=>x.stop===s.id);if(!list.length)return "";const picked=list.filter(x=>state.acts[s.id+"|"+x.name]);
  return `<h3 class="sec">Things to book <span class="cnt">${list.length}</span>${picked.length?`<span class="cnt on">${picked.length} in budget · $${picked.reduce((t,x)=>t+x.usd,0)}</span>`:""}</h3><div class="acts">${list.map(x=>{const on=!!state.acts[s.id+"|"+x.name];return `<button class="act-row ${on?"on":""} ${x.must?"must":""}" data-act="${esc(x.name)}"><span class="ai">${ACT_ICON[x.kind]||"✦"}</span><span class="at"><b>${esc(x.name)}${x.must?" <em>classic</em>":""}</b><small>${esc(x.note||"")}</small></span><span class="ap"><b>$${x.usd}</b><small>${x.days?x.days+" d":x.hours?x.hours+" h":""}</small></span><span class="ck">${on?"✓":"+"}</span></button>`;}).join("")}</div>`;}
function hoodsHTML(s){const hs=HOODS[s.id];if(!hs||!hs.length)return "";const norm=x=>(x||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"");
  return `<h3 class="sec">Which neighbourhood</h3><div class="hoods">${hs.map(x=>{const n=s.hostels.filter(h=>norm(h.area).includes(norm(x.name).split(/[ /(]/)[0])).length;return `<button class="hood ${n?"has":""}" data-hood="${esc(x.name)}"><div class="hh"><b>${esc(x.name)}</b><span class="bf">${esc(x.best_for||"")}</span>${n?`<span class="hn">${n} hostel${n>1?"s":""} ↓</span>`:""}</div><p>${esc(x.vibe)}</p><div class="pc"><span class="p">+ ${esc(x.pros)}</span><span class="c">− ${esc(x.cons)}</span></div><div class="nt">☾ ${esc(x.night)}</div></button>`;}).join("")}</div>`;}
function suggestNext(fromId){const on=new Set([...state.route.map(r=>r.id),"rio"]);const S=schedule();const last=S.rows.find(r=>r.id===fromId)||S.rows[S.rows.length-2];const month=last?addDays(last.to,1).getMonth()+1:state.wxMonth;const from=BY[fromId];const lastKinds=S.rows.slice(-4).map(r=>BY[r.id].kind);
  return STOPS.filter(s=>!on.has(s.id)&&s.id!=="rio").map(s=>{const lg=leg(fromId,s.id);if(lg.m==="flight"&&lg.h>6)return null;const w=wxAt(s,month);let sc=-lg.h/4+(lg.m==="flight"?-2.5:0)+(lg.est?-1.5:0)+(s.tier===1?3:s.tier===2?1.5:0)+(w?(w.r==="g"?2:w.r==="a"?0.5:-2):0)+(lastKinds.includes(s.kind)?-0.8:0.8)+(haversine(from,s)<80?-1:0);return {s,lg,w,sc};}).filter(Boolean).sort((a,b)=>b.sc-a.sc).slice(0,3);}
function renderSuggest(fromId){let box=document.getElementById("suggest");const list=suggestNext(fromId);if(!list.length){box&&box.remove();return;}if(!box){box=document.createElement("div");box.id="suggest";box.className="suggest";document.getElementById("mapwrap").append(box);}
  box.innerHTML=`<div class="sh"><span>Next from <b>${esc(short(BY[fromId].name))}</b>?</span><button id="sugX">×</button></div>${list.map(x=>`<button class="sg" data-add="${x.s.id}"><img src="${IMGT[x.s.id]||IMG[x.s.id]}" alt=""><span><b>${esc(short(x.s.name))}</b><small>${MODE_NAME[x.lg.m]||x.lg.m} ${x.lg.h>=24?Math.round(x.lg.h/24*10)/10+" d":x.lg.h+" h"} · ${x.s.days} d${x.w?" · "+x.w.hi+"°":""}</small></span><i class="wxd ${x.w?x.w.r:"g"}"></i></button>`).join("")}`;
  box.querySelector("#sugX").onclick=()=>box.remove();box.querySelectorAll("[data-add]").forEach(b=>{b.onclick=()=>{addStop(b.dataset.add);};b.onmouseenter=()=>{pinEls[b.dataset.add].classList.add("sel");};b.onmouseleave=()=>{if(state.sel!==b.dataset.add)pinEls[b.dataset.add].classList.remove("sel");};});}
function addStop(id,again){const s=BY[id];if(id==="rio")return;snap();
  ...
  state.route.push({id,days:s.days});commit();toast(`${short(s.name)} added · ${s.days} d`,undoLast);renderSuggest(id);}
...
(function(){const t=localStorage.getItem("andes2copa.theme");if(t)document.documentElement.dataset.theme=t;const cur=()=>document.documentElement.dataset.theme||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");
  document.getElementById("themeBtn").onclick=()=>{const n=cur()==="dark"?"light":"dark";document.documentElement.dataset.theme=n;localStorage.setItem("andes2copa.theme",n);document.querySelector('meta[name=theme-color]').setAttribute("content",n==="dark"?"#0F1D1C":"#EEF0EA");toast(n==="dark"?"Dark mode":"Light mode");};})();
...
document.addEventListener("keydown",e=>{if(story)return;if(e.target.matches&&e.target.matches("input,textarea"))return;
