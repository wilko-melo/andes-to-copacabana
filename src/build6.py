...
ids=re.findall(r'\{id:"(\w+)"',stops_js+open('data_stops_extra.js').read()+open('data_south.js').read()+open('data_remote.js').read())
...
T1=set(re.findall(r'"(\w+)"',open('data_stops_extra.js').read().split('const TIER1=[')[1].split(']')[0]))|set(re.findall(r'"(\w+)"',open('data_south.js').read().split('const TIER1_SOUTH=[')[1].split(']')[0]))
img2={};
for k in os.listdir("img2"):
    if k.endswith(".jpg"):
        sid=k.rsplit("_",1)[0]
        if LITE and sid not in T1: continue
        img2.setdefault(sid,[]).append("data:image/jpeg;base64,"+base64.b64encode(open("img2/"+k,"rb").read()).decode())
